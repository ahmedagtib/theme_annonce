/**
 * 
 *  I don't Inderstand this 2 badcodes
 * 
 */
...
$(function () {
    ...
    $('.newbtn').on("click" , function () {
        $(this).find('.pic').focus();
        console.log($(this).find('.pic'));
    });
    /** End Custom File image  */

    /** Start background color */

    $('textarea.form-control').css('background','#FFF');

    /** End background color */

    /* Start Gallery */

    var imgCount = $('.galery .thumbnails').children().length,

        marginBetween = '0.5',

        thumWidth = 100,

        totalMargin = (imgCount - 1) * marginBetween,

        imgWidth = (thumWidth - totalMargin) / imgCount;

    console.log(imgWidth);

    $('.galery .thumbnails img').css('width',imgWidth + '%');

    $('.galery .thumbnails img:not(:last-child)').css('margin-right',marginBetween + '%');

    $('.galery .thumbnails img').click(function () {

        $(this).addClass('selected').siblings().removeClass('selected');

        $('.showed-img img').hide().attr('src',$(this).attr("src")).fadeIn(500);
        
    });

    $('.galery .showed-img .prev-arrow').click(function () {

        if($('.selected').is(':first-child')) {

            $('.galery .thumbnails img').eq(imgCount - 1).click();
    ...
    $('.galery .showed-img .next-arrow').click(function () {

        if($('.selected').is(':last-child')) {

            $('.galery .thumbnails img').eq(0).click();
